...
export function AdminStatusFilters({
  ...
  function updateParam(key: string, value: string) {
    ...
    sp.delete("page")
    start(() => router.push(`/status?${sp.toString()}`))
  }

  function reset() {
    start(() => router.push("/status"))
  }
  ...
  return (
    <div className="flex flex-col gap-4 p-4 rounded-lg border border-border bg-card shadow-sm">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="space-y-1.5">
          <Label className="text-xs">ค้นหา (หน่วยงาน/ผู้ส่ง)</Label>
          <form
            onSubmit={(e) => {
              e.preventDefault()
          ...
          >
            <InputGroup>
              ...
              <InputGroupInput name="search" defaultValue={currentSearch} placeholder="คำค้นหา..." />
            </InputGroup>
          </form>
        </div>
      </div>
